...
void Kafelek::paintEvent(QPaintEvent *event)
{
    ...
    {
        QString napis = wpis.getTimeStart().toString("h.mm") + " " + wpis.getText();
...
void Kafelek::mousePressEvent(QMouseEvent *event)
{
    for (auto wpis: wpisy_kafelka)
    {
        ...
        connect(&dialog, &Dialog_kafelka::wydarzenie_accepted, this, &Kafelek::wydarzenie_accepted);
        dialog.exec();
    }
    Dialog_kafelka dialog(Wydarzenie(data, QTime(12,0), QTime (12,0),""), this);
    connect(&dialog, &Dialog_kafelka::wydarzenie_accepted, this, &Kafelek::wydarzenie_accepted);
    dialog.exec();
}
...
void Kafelek::dodaj_wpis(const Wydarzenie &w)
{
    for(const auto &wpis : wpisy_kafelka)
    {
        if(wpis.getTimeStart()== w.getTimeStart() and wpis.getTimeEnd()== w.getTimeEnd() and wpis.getDate() == w.getDate() and wpis.getText() == w.getText())
            return;
    }
